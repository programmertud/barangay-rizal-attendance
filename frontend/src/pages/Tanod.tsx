...
import type { TanodMember } from '../types/tanod';

const Tanod: React.FC = () => {
  const [members, setMembers] = useState<TanodMember[]>([]);
  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [formData, setFormData] = useState({ name: '', phone: '', shift: '', assignedPost: '', rfidUid: '' });
  ...
  const refreshMembers = async () => {
    try {
      const snapshot = await getDocs(collection(db, 'tanod'));
      const data = snapshot.docs.map((d) => ({ id: d.id, ...d.data() } as TanodMember));
      setMembers(data);
    } catch (error: any) {
  ...
  const startAdd = () => {
    setFormData({ name: '', phone: '', shift: '', assignedPost: '', rfidUid: '' });
    setPhotoFile(null);
    safeRevokeObjectUrl(photoPreviewUrl);
    setPhotoPreviewUrl('');
    setEditingId(null);
  ...
  const startEdit = (member: TanodMember) => {
    setFormData({
      name: member.name || '',
      phone: member.phone || '',
      shift: member.shift || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await updateDoc(doc(db, 'tanod', editingId), {
          ...payload,
          ...(photoDataUrl ? { photoDataUrl } : {}),
        });
      } else {
        const docRef = await addDoc(collection(db, 'tanod'), {
          ...payload,
          createdAt: new Date(),
        });

        if (photoDataUrl) {
          await updateDoc(doc(db, 'tanod', docRef.id), { photoDataUrl });
        }
      }
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await Promise.all(selectedIds.map((id) => deleteDoc(doc(db, 'tanod', id))));
      setSelectedIds([]);
      await refreshMembers();
    } catch (error) {
  ...
  const filteredMembers = members
    .filter((m) => {
      ...
      return (
        ...
        (m.assignedPost || '').toLowerCase().includes(q)
      );
    })
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[var(--text)]">Barangay Tanod</h1>
          <p className="text-sm text-[var(--muted)]">Manage your tanod roster</p>
        </div>
        <div>
      ...
      <Modal 
        ...
        title={editingId ? 'Edit Tanod' : 'Add New Tanod'}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="ui-label mb-2">Shift</label>
              <input
                type="text"
            ...
            <div>
              <label className="ui-label mb-2">Assigned Post</label>
              <input
                type="text"
                value={formData.assignedPost}
                onChange={(e) => setFormData({ ...formData, assignedPost: e.target.value })}
                className="ui-input"
              />
            </div>
